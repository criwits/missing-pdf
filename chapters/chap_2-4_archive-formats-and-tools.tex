...
其中 \MissingVerb{＿} 是一个空位，可以放一些别的字进去，这样一来，压缩过程可能又可以进一步省下空间——诗的好几句都有这个句式。在文件压缩技术中，这种「能匹配长相相近片段的模板」称为「模式」（pattern）。利用好模式信息，能有效地提升压缩的效率。
...
这些选项能帮助你解压和制作压缩包。例如，右图中【解压到 \MissingVerb{testdisk-7.2.win64\}】及【解压到此处】就可以将这个压缩文件解压，它们的区别早在\chapref{cha:file-and-file-management}中介绍过。而如果我们想将几个文件打包，也可以先选中它们，然后右键选择【新建压缩包】，并在弹出的窗口中选择压缩格式并设置压缩文件的名字，同时调整一些高级选项，比如给压缩文件设置密码。
